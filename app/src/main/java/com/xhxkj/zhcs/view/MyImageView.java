...
public class MyImageView extends ImageView {
     ...
     *
     * @param matrix Matrix是一个用于坐标变换的3*3矩阵
     * @param rolateX 旋转的X轴点
     * @param rolateY 旋转的Y轴点
     */
    private synchronized void BeginRolate(Matrix matrix, float rolateX,
        ...
        camera.save();
        camera.rotateX(this.rolateY > 0 ? rolateY : -rolateY);
        camera.rotateY(this.rolateX < 0 ? rolateX : -rolateX);
        camera.getMatrix(matrix);
        camera.restore();
        // 控制中心点
        if (this.rolateX > 0 && rolateX != 0) {
            matrix.preTranslate(-vWidth, -scaleY);
            matrix.postTranslate(vWidth, scaleY);
        } else if (this.rolateY > 0 && rolateY != 0) {
            matrix.preTranslate(-scaleX, -vHeight);
            matrix.postTranslate(scaleX, vHeight);
        } else if (this.rolateX < 0 && rolateX != 0) {
